    ...
    export default {
        ...
        data () {
            return {
                ...
                waveWidth2: 0.03, // 波浪宽度,数越小越宽
                waveHeight2: 3, // 波浪高度,数越大越高
                colors2: ['rgba(243, 156, 107, 0.48)', 'rgba(160, 86, 59, 0.48)'], // 波浪颜色
                xOffset2: 2, // 初始偏移
                speed2: 0.02, // 速度
            }
        },
        ...
        methods: {
            drawSin () {
                this.ctx.beginPath();
                for (let x = this.startX; x < this.startX + this.width; x += 20 / this.width ) {
            ...
            drawSin2 () {
                this.ctx.beginPath();
                for (let x = this.startX; x < this.startX + this.width; x += 20 / this.width ) {
                    const y = this.waveHeight2 * Math.sin((this.startX + x) * this.waveWidth2 + this.xOffset2);
                    this.points.push([x, (1 - this.nowRange) * this.height + y]);
                    this.ctx.lineTo(x, (1 - this.nowRange) * this.height + y);
                }
                this.ctx.lineTo(this.width, this.height);
